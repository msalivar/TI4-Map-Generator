...
- `serializeMapString()` (`js/app.js`) emits one token per non-Mecatol hex in
  ...
  otherwise, `"-1"` for an empty non-home hex. For the fixed 3-ring board that's
  always 36 tokens.
...
That string is the natural payload for a share URL. The only extra state needed
to reproduce a map is which expansion sets are enabled, because `parseMapString`
resolves bare ids and tile `id` is not globally unique across sets.

Out of scope of the map-string format (and therefore of this feature): player
names, locked tiles, ring count (board is a fixed `RINGS = 3`).
...
```
https://msalivar.github.io/TI4-Map-Generator/#m=0,0,-1,39,40,...&s=p,t,d
```

- `m=` — the 36 map-string tokens joined by `,` instead of the space that
  `serializeMapString()` uses. Same content, ~100–150 chars.
...
- No compression. 36 short tokens does not need it.

## Load behavior

New `loadFromHash()`, called as the first thing in `init()`:

1. Parse `location.hash`. Recognize it only if it has an `m=` param whose
   comma-split length equals `mapStringCells.length` (36). Otherwise return
   `false` and `init()` proceeds with the existing `localStorage` path unchanged.
2. Set `enabledSets` from the `s=` param (all-on if the param is absent; exactly
   the listed sets otherwise, including empty = base only).
3. Load the board from the tokens via `applyMapTokens(tokens, { persist: false })`
   (see Code changes).
4. Set a module-level `shareMode = true`.
5. Return `true`. `init()` skips the `localStorage` restore branch entirely and
   does not call `renderAll()` a second time (`applyMapTokens` already renders).
...
- **Refactor `parseMapString`** into two pieces:
  - `applyMapTokens(tokens, { persist, partial })` — the resolve-and-place core:
    ...
    - `partial: false` (Import button) — preserves today's all-or-nothing
      behavior: if any token is unresolved, return the bad tokens and leave
      `board`/`pool` untouched.
    - `partial: true` (hash load) — place every token that resolves, skip the
      rest, still return them.
    - `persist()` is called at the end only when `persist` is true **and** the
      board was actually applied.
  - `parseMapString(str)` — keeps the whitespace split, the token-count check,
    and the `alert()` on bad tokens; delegates placement to
    `applyMapTokens(tokens, { persist: true, partial: false })`. The Import Map
    String button is otherwise unchanged.
...
- **Set-code helpers**:
  ...
  - `decodeSetsParam(str)` — inverse; unknown codes ignored.
  - Single source-of-truth mapping object `{ p: "pok", t: "thunders-edge",
    d: "discordant-stars" }` used both directions.
- **`buildShareURL()`** —
  `location.origin + location.pathname + "#m=" +
  serializeMapString().split(" ").join(",") + "&s=" +
  encodeSetsParam(enabledSets)`.
- **`loadFromHash()`** — as described in Load behavior; called first in `init()`,
  gating the existing `saved`/`loadFromObject` branch.
...
6. **`s=` absent** — a link with only `m=`: all expansion sets end up enabled and
   the map loads.
7. **`s=` empty / partial** — `&s=` (base only) and `&s=p` load with exactly
   those sets enabled.
8. **Clipboard fallback** — with `navigator.clipboard` unavailable, the button
