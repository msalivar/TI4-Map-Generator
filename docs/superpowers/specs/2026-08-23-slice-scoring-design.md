...
- No distance-to-Mecatol point adjustment. In ti4-lab, home distance to Mecatol can vary by slice shape, so a "closer/farther than baseline" adjustment differentiates players. In this app, each home sits at some fixed hex-distance from Mecatol — usually, but not always, equal to the board's ring count (e.g. 3-Player Settlers' homes are at distance 2 on a rings:3 board, and 7-Player Alt Hyperlanes' at distance 3 on a rings:4 board; see `homePathTiles()` in `js/scoring.js` for how the path is computed generically from each home's own true distance). Every home in a given layout is the same distance from Mecatol as every other, though, so that differentiating term would always compute to exactly 0 here, and it's dropped entirely rather than implemented as dead code.
