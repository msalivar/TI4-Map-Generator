...
- **Export/Import format**: "map strings", not JSON — one number per hex in ring order (`cells.slice(1)`, skipping Mecatol Rex since it's always the center), `"0"` for a home system, a tile's `id` otherwise, `"-1"` for an empty non-home hex. This is the same convention used by other community TI4 map tools; `cells.slice(1)`'s natural order already matches it (verified against a real external example), so don't reorder it when touching `serializeMapString`/`parseMapString`.
